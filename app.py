...
if df_raw is not None:
    ...
    if st.button("🔥 전체 품목 전문 Gemini 분석 시작"):
        ...
        if not os.environ.get("GEMINI_API_KEY") or not os.environ.get("SERPER_API_KEY"):
            ...
            with st.status("Gemini 분석팀이 뉴스 기반 예측 보고서를 작성 중입니다...", expanded=True) as status:
                analyst = Agent(
                    role="시장 수급 및 단가 예측 전문가",
                    goal="최신 뉴스와 데이터를 기반으로 품목별 향후 1~3개월 단가 예측",
                    backstory="뉴스 근거를 바탕으로 단가 변동의 인과관계를 밝히는 수석 분석가입니다.",
                    llm=gemini_llm,
                    tools=[search_tool],
                    verbose=True
                )
                ...
                final_report_md += f"본 보고서는 Gemini 엔진을 통해 총 **{len(critical_items)}개** 품목을 정밀 분석했습니다.\n\n"
                final_report_md += "\n\n---\n\n".join(all_reports)
            ...
            st.download_button(label="📄 Gemini 전문 보고서 다운로드 (Word)", data=docx_file, file_name=f"Gemini_Market_Report_{datetime.date.today()}.docx")
