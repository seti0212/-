...
# ============================================================================
# 1. 환경 및 보안 설정 (사이드바 입력창 제거)
# ============================================================================
st.set_page_config(page_title="구매지원팀 통합 분석 시스템 (Gemini)", layout="wide")

# [보안] Streamlit Secrets에서만 키를 로드합니다. (화면 노출 없음)
# 배포 후 Streamlit Cloud 앱 설정의 Secrets 항목에 아래 키들을 등록하세요.
# GEMINI_API_KEY = "내_제미나이_키"
# SERPER_API_KEY = "내_서퍼_키"
...
def get_critical_items(w_df, m_df, y_df):
    """주/월/연 전체의 모든 상승/하락 이슈 품목을 중복 없이 수집"""
...
if df_raw is not None:
    ...
    st.caption("※ 모든 상승/하락 이슈 품목의 최신 뉴스를 검색하여 단가 변동 근거와 예측을 수행합니다.")

    if st.button("🔥 전체 품목 전문 Gemini 분석 시작"):
        # 키 설정 여부 확인
        if not os.environ.get("GEMINI_API_KEY") or not os.environ.get("SERPER_API_KEY"):
            st.error("🚨 API 키가 누락되었습니다. Streamlit Secrets 설정을 완료해 주세요.")
        else:
            search_tool = SerperDevTool()
            # Gemini 모델 설정 (Flash 모델로 속도 최적화)
            gemini_llm = LLM(model="gemini/gemini-1.5-flash")

            with st.status("Gemini 분석팀이 뉴스 검색 및 단가 예측 리포트를 작성 중입니다...", expanded=True) as status:
                analyst = Agent(
                ...
                procurement = Agent(
                    ...
                    backstory="원가 절감 및 공급 안정성을 위한 실질적 구매 액션 플랜을 설계합니다.",
                    llm=gemini_llm,
                    verbose=True
                )
                ...
                for idx, item in enumerate(critical_items):
                    st.write(f"⏳ ({idx+1}/{len(critical_items)}) **{item}** 정밀 분석 중...")
                    
                    t1 = Task(
                        description=f"{item}의 최근 급등락 원인을 최신 뉴스에서 찾아 분석하고, 향후 1~3개월 단가(상승/하락/보합)를 뉴스 근거와 함께 예측하세요.",
                    ...
                    # Gemini는 속도가 빠르므로 max_rpm을 넉넉히 설정
                    ...
                    time.sleep(1) # 안정성을 위한 짧은 휴식
                    progress_bar.progress((idx + 1) / len(critical_items))

                final_report_md = f"# 📑 구매부서 종합 이슈 보고서 ({datetime.date.today()})\n\n"
                final_report_md += f"본 보고서는 Gemini 엔진을 통해 총 **{len(critical_items)}개** 이슈 품목을 분석했습니다.\n\n"
                final_report_md += "\n\n---\n\n".join(all_reports)
            ...
            st.download_button(label="📄 Gemini 전문 보고서 다운로드 (Word)", data=docx_file, file_name=f"Market_Report_{datetime.date.today()}.docx")
