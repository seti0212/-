...
# ============================================================================
# 1. 환경 및 보안 설정 (사이드바 제거 및 Gemini 호환성 최적화)
# ============================================================================
st.set_page_config(page_title="구매지원팀 통합 분석 시스템 (Gemini)", layout="wide")

# [보안] Streamlit Secrets에서 키 로드 및 모든 환경변수 대응
if "GEMINI_API_KEY" in st.secrets:
    api_key = st.secrets["GEMINI_API_KEY"]
    os.environ["GEMINI_API_KEY"] = api_key
    os.environ["GOOGLE_API_KEY"] = api_key  # 라이브러리에 따라 이 이름을 요구함
...
if df_raw is not None:
    ...
    # ============================================================================
    # 4. 전문 AI 분석 섹션 (404 에러 수정 및 안정성 강화)
    # ============================================================================
    ...
    if st.button("🔥 전체 품목 전문 Gemini 분석 시작"):
        if not os.environ.get("GEMINI_API_KEY") or not os.environ.get("SERPER_API_KEY"):
            st.error("🚨 API 키 설정(Secrets)을 확인해 주세요.")
        else:
            search_tool = SerperDevTool()
            
            # [404 에러 방지 핵심 설정]
            # model 이름을 'gemini/gemini-1.5-flash'로 변경하여 경로 충돌을 방지합니다.
            gemini_llm = LLM(
                model="gemini/gemini-1.5-flash",
                api_key=os.environ["GEMINI_API_KEY"]
            )

            with st.status("Gemini 분석팀이 보고서를 작성 중입니다...", expanded=True) as status:
                analyst = Agent(
                    role="시장 수급 및 단가 예측 전문가",
                    goal="최신 뉴스와 데이터를 기반으로 품목별 향후 1~3개월 단가 방향성 예측",
                    backstory="뉴스 근거를 바탕으로 단가 변동 원인을 명확히 밝히는 분석가입니다.",
                    llm=gemini_llm,
                    tools=[search_tool],
                    verbose=True
                )
                ...
                for idx, item in enumerate(critical_items):
                    ...
                    crew = Crew(agents=[analyst, procurement], tasks=[t1, t2], max_rpm=5, verbose=True)
